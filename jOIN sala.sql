...
-- ATIVIDADE SALA 2026-03-18
                     
                     -- RELATORIO DE VENDAS VALIDAS --
-- Gerencia da area de vendas da empresa de sucos de frutas quer um relatorio que mostre dentre as venda, quais foram validas e quais delas foram invalidas 
-- 1° Obter o volume total de vendas para cada dia cliente dentro de 1 mês

SELECT * FROM notas_fiscais;
SELECT * FROM itens_notas_fiscais;

--  Junção das duas tabelas com JOIN
SELECT NF.CPF, NF.DATA_VENDA, INF.QUANTIDADE FROM notas_fiscais AS NF INNER JOIN itens_notas_fiscais AS INF ON NF.NUMERO = INF.NUMERO; 
...
SELECT NF.CPF, DATE_FORMAT(NF.DATA_VENDA, '%y-%m' ) AS ANO_MES, SUM(INF.QUANTIDADE) AS QNTD_VENDAS
FROM notas_fiscais AS NF INNER JOIN itens_notas_fiscais AS INF ON NF.NUMERO = INF.NUMERO GROUP BY NF.CPF,DATE_FORMAT(NF.DATA_VENDA, '%y-%m');
...
-- Subquery
SELECT X.CPF, X.NOME, X.ANO_MES, X.QNTD_VENDAS, X.QNTD_LIMITE FROM (SELECT NF.CPF, TC.NOME, DATE_FORMAT(NF.DATA_VENDA, '%y-%m') AS ANO_MES, SUM(INF.QUANTIDADE)
AS QNTD_VENDAS, TC.VOLUME_DE_COMPRA AS QNTD_LIMITE FROM notas_fiscais AS NF INNER JOIN itens_notas_fiscais AS INF ON NF.NUMERO = INF.NUMERO INNER JOIN tabela_de_clientes AS TC on TC.CPF = NF.CPF GROUP BY NF.CPF, TC.NOME, 
DATE_FORMAT(NF.DATA_VENDA, '%y-%m')) AS X;

-- Fim Atividades em Sala...
